// Benchmarks vector dot products and norms.
...
void BenchmarkDot(benchmark::State& state)
{
    auto a = bench::MakeRandomVector<Lib, N>(42);
    ...
    state.SetItemsProcessed(state.iterations() * (int64_t{2} * N - 1));
    state.SetBytesProcessed(state.iterations() *
        int64_t{2} * N * sizeof(typename Lib::scalar_t));
    state.SetLabel(std::string(Lib::kName) + "/N=" + std::to_string(N));
}

template<class Lib, std::size_t N>
void BenchmarkNormL2(benchmark::State& state)
{
...
template<class Lib, std::size_t N>
void BenchmarkNormL1(benchmark::State& state)
{
    auto v = bench::MakeRandomVector<Lib, N>(42);
    for (auto _ : state) {
        auto r = Lib::NormL1(v);
...
template<class Lib, std::size_t N>
void BenchmarkNormLInf(benchmark::State& state)
{
    auto v = bench::MakeRandomVector<Lib, N>(42);
    for (auto _ : state) {
        auto r = Lib::NormLInf(v);
        benchmark::DoNotOptimize(r);
    }
    state.SetItemsProcessed(state.iterations() * int64_t{N});
    state.SetBytesProcessed(state.iterations() * int64_t{N} * sizeof(typename Lib::scalar_t));
...
REG_T(BenchmarkDot,      float);  REG_T(BenchmarkDot,      double);
REG_T(BenchmarkNormL2,   float);  REG_T(BenchmarkNormL2,   double);
REG_T(BenchmarkNormL1,   float);  REG_T(BenchmarkNormL1,   double);
REG_T(BenchmarkNormLInf, float);  REG_T(BenchmarkNormLInf, double);
